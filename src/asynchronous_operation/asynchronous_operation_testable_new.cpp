...
/*std::net::io_context::executor_type mock_async_write_stream::get_executor() noexcept {
  return ex_;
}*/
...
std::error_code ec;
std::size_t bytes = 0;
async_wait_then_write(timer, stream, std::chrono::milliseconds(1000), buffer(in), [&] (auto e, auto b) { i = true; ec = e; bytes = b; });
assert(!i);
